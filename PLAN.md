...
- [ ] 将路线可行性从推荐排序中分离为确定性规则引擎。
- [ ] 校验段间地点连通、绝对时间顺序、最短换乘时间、接驳时间和风险缓冲。
- [ ] 为国内/国际、同机场/跨机场、火车转飞机定义不同最短衔接规则。
- [ ] 输出 `feasible`、`infeasible`、`uncertain` 及结构化原因。
- [ ] `infeasible` 路线不得进入最终 Top 5；`uncertain` 必须明确提示风险。
...
- 无接驳证据的跨城组合不会生成候选路线。
- 2026-07-28：完整自动化测试 143 项通过，`compileall` 与 `git diff --check` 通过。

## 当前执行点

P0.1–P0.3 已完成。下一项是 **P0.4 建立可行性引擎**；开始前先读取本计划，并保持可行性判断与推荐排序分离。
